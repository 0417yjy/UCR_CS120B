...
unsigned char switchSpeaker = 0x00;
...
int TickFct_CombineLEDs (int state) {
	...
	switch(state) {
		case CL_init:
			break;
		case CL_working:
			PORTB = threeLEDS | (blinkingLED << 3) | (switchSpeaker << 4);
			break;
	}
	return state;
};
enum SwitchSpeakerStates {SS_init, SS_off, SS_change_frequency, SS_on};
int TickFct_SwitchSpeaker (int state) {
...
int main(void) {
	...
	i++;
	tasks[i].state = SS_init;
	tasks[i].period = 1;
	tasks[i].elapsedTime = tasks[i].period;
	tasks[i].TickFct = &TickFct_SwitchSpeaker;
	i++;
