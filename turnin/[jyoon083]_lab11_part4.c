...
enum KeypadLCDStates {KL_init, KL_wait, KL_pressed};
int TickFct_KeypadLCD(int state) {
	const char *init_str = "Congratulations!";
	unsigned char x;
	static char display_idx = 0;
	
	switch(state) { // Transitions
		case KL_init:
			LCD_DisplayString(1, init_str);
			state = KL_wait;
			break;
		case KL_wait:
			x = GetKeypadKey();
